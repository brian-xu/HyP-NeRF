...
class PSNRMeter:
    ...
    def report(self, prefix = ""):
        return f'{prefix}PSNR = {self.measure():.6f}'
...
class Trainer(object):
    def __init__(self, 
        ...
        self.results_save_path = self.workspace
    ...
    def train_step(self, _data):
        ...
        rays_o = data['rays_o'].squeeze(1) # [B, N, 3]
        rays_d = data['rays_d'].squeeze(1) # [B, N, 3]

        images = data['images'].squeeze(1) # [B, N, 3/4]
    ...
    def eval_step(self, _data):
        ...
        rays_o = data['rays_o'].squeeze(1) # [B, N, 3]
        rays_d = data['rays_d'].squeeze(1) # [B, N, 3]
        images = data['images'].squeeze(1) # [B, H, W, 3/4]
        ...
        return pred_rgb, pred_depth, pred_mask, gt_rgb, loss

    # moved out bg_color and perturb for more flexible control...
    def test_step(self, _data, conditional_index=None, bg_color=None, perturb=False): 
        ...
        if conditional_index is not None:
            index = torch.LongTensor([conditional_index]).to(self.device)
    ...
    def save_mesh(self, save_path=None, resolution=256, threshold=10, index=206):

        if save_path is None:
            save_path = os.path.join(self.results_save_path, 'meshes', f'{self.name}_{self.epoch}.ply')
        ...
        print(index.shape)
        z_shape = self.model.shape_code(index)
        z_color = self.model.color_code(index)
        params = self.model.hyper_net(z_shape, z_color)
        def query_func(pts):
            with torch.no_grad():
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    sigma = self.model.net.density(pts.to(self.device),params=params)['sigma']
            return sigma

        vertices, triangles = extract_geometry(self.model.net.aabb_infer[:3], self.model.net.aabb_infer[3:], resolution=resolution, threshold=threshold, query_func=query_func)
    ...
    def train(self, train_loader, valid_loader, max_epochs, ckpt_path=None):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.epoch % self.eval_interval == 0 or self.epoch == 1:
                print("Saving results and the checkpoints...")
                self.evaluate_one_epoch(valid_loader)
                
                if ckpt_path is not None:
                    self.save_checkpoint(ckpt_path=ckpt_path, full=True, best=False)
                else:
    ...
    def test(self, loader, conditional_index=None, save_path=None, name=None, write_video=True):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth = self.test_step(data, conditional_index)
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        num_validation_examples = self.opt.num_validation_examples
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_step > num_validation_examples:
                    break

                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth, pred_masks, truths, loss = self.eval_step(data)
                ...
                if self.local_rank == 0:
                    ...
                    object_id, pose = data[0]["filename"][0].split("_")
                    save_fold = os.path.join(self.results_save_path, 'validation', object_id)

                    prefix = os.path.join(save_fold, f'{name}_{self.local_step:04d}_{pose}')
                    ...
                    mask = pred_masks[0].detach().cpu().numpy()
                    mask = (mask * 255).astype(np.uint8)
                    
                    cv2.imwrite(f'{prefix}_rgb.png', cv2.cvtColor(pred, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(f'{prefix}_truth.png', cv2.cvtColor(truth, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(f'{prefix}_depth.png', pred_depth)
        ...
        if self.local_rank == 0:
            ...
            for metric in self.metrics:
                self.log(metric.report(prefix = "Compression "), style="blue")
                if self.use_tensorboardX:
    ...
    def save_checkpoint(self, ckpt_path=None, name=None, full=False, best=False, remove_old=True):
        full = True #Hardcoding this as we always want to save the optimizer.
        ...
        if not best:
            ...
            if not self.opt.test:
                ...
                file_path = f"{ckpt_path}/{name}.pth"
            # file_path = f"{self.ckpt_path}/{name}.pth"

            if self.opt.remove_old:
            ...
            print(f'Saving checkpoint at : {file_path}')
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        ...
        # else:
        #     assert False, "optimizer not loaded!!"
